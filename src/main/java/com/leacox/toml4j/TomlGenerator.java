...
public class TomlGenerator {
	...
	private void writeTo(BufferedOutputStream outputStream, TomlNode node) throws IOException {
		...
		case STRING:
			String stringValue = new StringBuilder().append("\"").append(StringUtils.escapeString(node.stringValue()))
					.append("\"").toString();
	...
	private void writeArray(BufferedOutputStream outputStream, TomlArrayNode array) throws IOException {
	...
	private void writeHash(BufferedOutputStream outputStream, TomlHashNode hash, String keyGroup) throws IOException {
		if (keyGroup != null) {
			String keyGroupLine = new StringBuilder().append("[").append(keyGroup).append("]\n").toString();
		...
		for (; fields.hasNext();) {
			...
			if (node.isValueNode() || node.isArray()) {
				...
				String childKeyGroup = keyGroup == null ? key : new StringBuilder(keyGroup).append(".").append(key)
						.toString();
